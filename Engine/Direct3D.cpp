...
HRESULT Direct3D::InitShader3D()
{
	...
	D3D11_INPUT_ELEMENT_DESC layout[] = 
	{
		...
		{ "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, sizeof(DirectX::XMFLOAT3) , D3D11_INPUT_PER_VERTEX_DATA, 0 },//UV座標
		{ "NORMAL",	0, DXGI_FORMAT_R32G32B32_FLOAT, 0, sizeof(DirectX::XMFLOAT3) * 2 ,	D3D11_INPUT_PER_VERTEX_DATA, 0 },//法線
	};
...
HRESULT Direct3D::InitShaderOutLine()
{
	...
	//ラスタライザ作成
	D3D11_RASTERIZER_DESC rdc = {};
	rdc.CullMode = D3D11_CULL_FRONT;  //多角形の裏側
	rdc.FillMode = D3D11_FILL_SOLID; //多角形の内部を塗りつぶす
	rdc.FrontCounterClockwise = FALSE; //反時計回りを表にするかどうか（がfalseなので時計回りが表）
...
HRESULT Direct3D::InitShaderStage()
{
	HRESULT hr;
	...
	// 頂点シェーダの作成（コンパイル）
	ID3DBlob* pCompileVS = nullptr;
	D3DCompileFromFile(L"Engine\\StageShader.hlsl", nullptr, nullptr, "VS", "vs_5_0", NULL, 0, &pCompileVS, NULL);
	assert(pCompileVS != nullptr);
	hr = pDevice->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL,
		&shaderBundle[SHADER_TYPE::SHADER_STAGE].pVertexShader);
	if (FAILED(hr))
	{
		MessageBox(NULL, "頂点シェーダの作成に失敗", NULL, MB_OK);
		return hr;
	...
	hr = pDevice->CreateInputLayout(layout, 3, pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(),
		&shaderBundle[SHADER_TYPE::SHADER_STAGE].pVertexLayout);
	...
	// ピクセルシェーダの作成（コンパイル）
	ID3DBlob* pCompilePS = nullptr;
	D3DCompileFromFile(L"Engine\\StageShader.hlsl", nullptr, nullptr, "PS", "ps_5_0", NULL, 0, &pCompilePS, NULL);
	assert(pCompileVS != nullptr);
	hr = pDevice->CreatePixelShader(pCompilePS->GetBufferPointer(), pCompilePS->GetBufferSize(), NULL,
		&shaderBundle[SHADER_TYPE::SHADER_STAGE].pPixelShader);
	pCompilePS->Release();
	if (FAILED(hr))
	{
		MessageBox(NULL, "ピクセルシェーダの作成に失敗", NULL, MB_OK);
	...
	rdc.FillMode = D3D11_FILL_SOLID; //多角形の内部を塗りつぶす
	rdc.FrontCounterClockwise = FALSE; //反時計回りを表にするかどうか（がfalseなので時計回りが表）
	hr = pDevice->CreateRasterizerState(&rdc, &shaderBundle[SHADER_TYPE::SHADER_STAGE].pRasterizerState);
	if (FAILED(hr))
	{
		MessageBox(NULL, "ラスタライザステートの作成に失敗", NULL, MB_OK);
		return hr;
